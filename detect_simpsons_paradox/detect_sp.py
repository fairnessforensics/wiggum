...
class _trendDetectors():
    ...
    def get_SP_views(self,sp_type='SP',
    ...
    def get_SP_rows(self,sp_type='SP',
                cols_pair = ['agg_trend','subgroup_trend'],
                colored=False, sp_args = None,inplace=False):
        """
        ...
        if not col_name in results_df.columns:
            self.label_SP_rows(sp_type,cols_pair, sp_args)

        # filter
        sp_df = self.results_df[results_df[col_name] == True]

        if inplace:
            self.results_df = sp_df
