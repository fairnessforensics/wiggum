...
def label_SP_rows(results_df,sp_type='SP',
                    cols_pair = ['agg_trend','subgroup_trend'],
                    sp_args = None):
    """
    return a DataFrame with an additional colulmn indicateing rows with SP
...
def get_SP_views(results_df,sp_type='SP',
            cols_pair = ['agg_trend','subgroup_trend'],
            colored=False):
    """
...
def get_views(results_df,colored=False):
    """
    return a list of tuples of the views of the dataset that have at least one
    occurence of SP. Assumes no views are listed in in opposite orders
    ...
    if colored:
        views_per_occurence = zip(results_df.feat1.values,
                                          results_df.feat2.values,
                                          results_df.group_feat.values)
        sp_views_unique = set([(a,b,c) for a,b,c in views_per_occurence])
    else:
