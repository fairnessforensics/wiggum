...
class _resultDataFrame():
    ...
    def get_views(self,colored=False):
        """
        ...
        if colored:
            views_per_occurence = zip(self.results_df.feat1.values,
                                        self.results_df.feat2.values,
                                        self.results_df.group_feat.values)
    ...
    def add_view_count(self,count_df,colored=False):
        # TODO: is this right for private method?
        """
        ...
        return self.result_df.merge(count_df, left_on = view_vars, right_on=view_vars)

    def add_view_score(self,score_col,agg_type,colored=False):
        """
        add the view counts to a result df for occurence ranking based on view rank
        ...
        self.result_df.merge(view_df, left_on = view_vars, right_on=view_vars,
                            suffixes=['', suffix])
    ...
    def get_trend_row(self,feat1,feat2,subgroup):
        """
    ...
    def count_sp_views(results_df, colored= False, portions =False,data_df= None,
        ...
        count_df = self.results_df.groupby(by=view_vars)['subgroup'].count().reset_index()
        ...
        if groupby_count :
            ...
            gby_df = self.results_df.groupby(uncolored_view_vars)[color_var]
        ...
        if portions :
            ...
            group_feat_list = set(self.results_df.group_feat.values)
        ...
        if append_counts:
            return_df = self.add_view_count(count_df,colored)
        else:
            return_df = count_df
    ...
    def rank_occurences_by_view(self,view_score=None,occurence_score=None):
        ...
        if view_score is None:
            # add slope
            view_score = 'all_slope'

            if not(view_score in result_df.columns):
                # then compute and augmented
                # add slope
                self.result_df = add_slope_cols(data_df,result_df)
                # add angle
        ...
        if occurence_score is None:
            # add slope
            occurence_score = 'subgroup_slope'
        ...
        # sort by rank_col
        self.result_df = self.result_df.sort_values(by=[view_score,occurence_score])
