...
const country_map_projection_center_virtual_layer = (selection, props) => {
	const {
	  side,
	  thirdLevelParentVLWidth,
	  level
	...
	d3.json("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson")
		...
		selection.each(function (d) {
			...
			// Draw curve area
			center_point_y = projection(geoCentroid)[1];
			var curve_point = [
				{ start1: { x: -(rectWidth/2 + 50 + thirdLevelParentVLWidth), y: 10 + poistion_y }, 
					start2: { x: -(rectWidth/2 + 50 + thirdLevelParentVLWidth), y: 30 + poistion_y }, 
					end1: { x: -10, y: center_point_y - 10 }, 
					end2: { x: -10, y: center_point_y + 10 },
					subgroup: subgroup }
			];
